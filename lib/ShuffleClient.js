...
class ShuffleClient extends EventEmitter {
  ...
  cleanupCompletedRound(shuffleRoundObject) {
    if (!shuffleRoundObject) {
      console.trace('emit error from cleanupCompletedRound');
      this.emit('error', new Error(`Round failed`));
      return;
    }
    ...
    // this library.
      ...
      this.emit('error', new Error(`Round error ${shuffleRoundObject.roundError.shortCode}`));
    }
